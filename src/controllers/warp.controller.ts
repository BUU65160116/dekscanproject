...
/** ===== Validation / Anti-Spam config & helpers (ย้ายจาก router มาไว้ที่นี่) ===== */
...
/** ===== Controller สำหรับ “แจกวาป” ===== */
export class WarpController {
  /** GET /warp (หน้า UI ลูกค้า) */
  static async renderWarpPage(_req: Request, res: Response) {
    return res.render("warp", { title: "แจกวาป" });
  }

  /**
   * GET /warp/credits
   * - ใช้ tableId จาก session เท่านั้น
   * - ถ้ามี ?recalc=1 → รีคาลก์จาก Odoo ก่อนแล้วคืนค่า
   * response: { ok, data: { total, used, left } }
   */
  static async getCredits(req: Request, res: Response) {
    try {
      ...
      const data = doRecalc
        ? await recalcAndGetLeft(tableId)
  ...
  /**
   * POST /warp/use
   * - ใช้ customerId/tableId จาก session เท่านั้น
   * - body: { message?: string, imageUrl?: string }
   * response: { ok, left, queueId }
   */
  static async useOne(req: Request, res: Response) {
    try {
